...
async function showPredictions(batch) {
  ...
  tf.tidy(() => {
    ...
    ui.showTestResults(predictions[0]);
  });
}
...
function init() {
    ...
    canvasElement.onmouseleave = handleMouseLeave;
    ...
    function handleMouseUp(e) {
        ...
        img.onload = function(){
            ...
            resizeCanvasCtx.drawImage(img, 0, 0, 28, 28);
            const imgData = resizeCanvasCtx.getImageData(0, 0, 28, 28);
            const datasetBytesView = [];
            for (let i = 0; i < imgData.data.length / 4; i++) {
                datasetBytesView[i] = imgData.data[i * 4] / 255;
            }
            const batchImagesArray = new Float32Array(datasetBytesView);
            const batch = {
                xs: tf.tensor2d(batchImagesArray, [1, 784]),
            };
            showPredictions(batch);
        };
